...
@interface WithdrawalsVCModule ()<UITextFieldDelegate,UIPickerViewDataSource,UIPickerViewDelegate,MoxieSDKDelegate>
{
...
@implementation WithdrawalsVCModule
...
-(void)loadWithdrawalsView:(DrawingsInfoModel *) drawingsInfo{
    ...
    if ([_drawingsInfoModel.productId isEqualToString:RapidLoan] || [_drawingsInfoModel.productId isEqualToString:DeriveRapidLoan]) {
        ...
        NSString *amountText = [NSString stringWithFormat:@"到期还款: %.2f元", [_drawingsInfoModel.repaymentAmount floatValue]];
...
-(void)withdrawalsViewAgreement:(DrawingsInfoModel *) drawingsInfo{
    ...
    //协议分为合规  和发薪贷
    if ([drawingsInfo.platformType isEqualToString:@"0"]||[drawingsInfo.platformType isEqualToString:@"3"]) {
...
-(void)pushFeeDescription{
    ...
    FeesDescriptionModules * feeDescVC = [[FeesDescriptionModules alloc]init];
...
-(void)getBankListInfo{
    ...
    [checkBankViewModel setBlockWithReturnBlock:^(id returnValue) {
        ...
        if ([baseResult.flag isEqualToString:@"0000"]) {
            ...
            HG_OpenAccountAddBankCardModules *bankVC = [HG_OpenAccountAddBankCardModules new];
