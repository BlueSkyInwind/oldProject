...
-(void)getPersonalCenterInfo{
    [[HF_NetWorkRequestManager sharedNetWorkManager]GetWithURL:[NSString stringWithFormat:@"%@%@",_main_new_url,_PersonalCenterWithdrawCashAPI_url] isNeedNetStatus:true isNeedWait:true parameters:nil finished:^(EnumServerStatus status, id object) {
...
-(void)withdrawCashAmount:(NSString *)amount bankCardId:(NSString *)bankCardId operateType:(NSString *)operateType payPassword:(NSString *)payPassword redpacketId:(NSString *)redpacketId{
    ...
    [[HF_NetWorkRequestManager sharedNetWorkManager]DataRequestWithURL:[NSString stringWithFormat:@"%@%@",_main_new_url,_WithdrawCash_url] isNeedNetStatus:true isNeedWait:true parameters:paramDic finished:^(EnumServerStatus status, id object) {
        if (self.returnBlock) {
            self.returnBlock(object);
        }
    } failure:^(EnumServerStatus status, id object) {
...
-(void)withdrawCashDetailOperateType:(NSString *)operateType pageNum:(NSString *)pageNum pageSize:(NSString *)pageSize{
    ...
    [[HF_NetWorkRequestManager sharedNetWorkManager] GetWithURL:[NSString stringWithFormat:@"%@%@",_main_new_url,_WithdrawCashDetail_url] isNeedNetStatus:true isNeedWait:true parameters:paramDic finished:^(EnumServerStatus status, id object) {
        if (self.returnBlock) {
            self.returnBlock(object);
        }
    } failure:^(EnumServerStatus status, id object) {
