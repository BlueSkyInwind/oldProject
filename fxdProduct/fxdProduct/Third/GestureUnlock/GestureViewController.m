...
@interface GestureViewController ()<CircleViewDelegate,DismissDelegate>
...
- (void)setupSubViewsLoginVc
{
    ...
    imageView.center = CGPointMake(kScreenW/2, kScreenH/4);
    [imageView setImage:[UIImage imageNamed:@"lock_head"]];
...
//    [self creatButton:leftBtn frame:CGRectMake(CircleViewEdgeMargin + 20, kScreenH - 60, kScreenW/2, 20) title:@"管理手势密码" alignment:UIControlContentHorizontalAlignmentLeft tag:buttonTagManager];
    ...
    [self creatButton:rightBtn frame:CGRectMake(0, 0, 100, 20) title:@"登陆其他账户" alignment:UIControlContentHorizontalAlignmentCenter tag:buttonTagForget];
...
- (void)creatButton:(UIButton *)btn frame:(CGRect)frame title:(NSString *)title alignment:(UIControlContentHorizontalAlignment)alignment tag:(NSInteger)tag
{
    ...
    btn.center = CGPointMake(kScreenW/2, kScreenH-50);
...
- (void)circleView:(PCCircleView *)view type:(CircleViewType)type didCompleteSetSecondGesture:(NSString *)gesture result:(BOOL)equal
{
    ...
    if (equal) {
        ...
        [self.delegate setSwithchState:YES];
        [[MBPAlertView sharedMBPTextView] showTextOnly:[UIApplication sharedApplication].keyWindow message:@"设置成功"];
        [self.navigationController popViewControllerAnimated:YES];
...
- (void)circleView:(PCCircleView *)view type:(CircleViewType)type didCompleteLoginGesture:(NSString *)gesture result:(BOOL)equal
{
    ...
    if (type == CircleViewTypeLogin) {
        ...
        if (equal) {
            DLog(@"登陆成功！");
            [[FXDNetWorkManager sharedNetWorkManager] POSTWithURL:[NSString stringWithFormat:@"%@%@",_main_url,_login_url] parameters:[self getParamDic] finished:^(EnumServerStatus status, id object) {
                _loginParse = [LoginBaseClass modelObjectWithDictionary:object];
                DLog(@"---%@",_loginParse);
                if ([_loginParse.flag isEqualToString: @"0000"]) {
                    [Utility sharedUtility].userInfo.userInfoModel = _loginParse;
                    //登录统计(账号)
                    [MobClick profileSignInWithPUID:userTableName];
                    //打开数据库
                    [[DataBaseManager shareManager] dbOpen:userTableName];
                    
                    [[MBPAlertView sharedMBPTextView] showTextOnly:self.view message:@"登录成功"];
                    [self dismissViewControllerAnimated:YES completion:nil];
                    ((AppDelegate *)[UIApplication sharedApplication].delegate).btb.selectedIndex = 0;
                    [((AppDelegate *)[UIApplication sharedApplication].delegate).btb setTabbarCon];
                } else {
                    if ([_loginParse.flag isEqualToString:@"0002"]) {
                        
                        [[HHAlertViewCust sharedHHAlertView] showHHalertView:HHAlertEnterModeTop leaveMode:HHAlertLeaveModeBottom disPlayMode:HHAlertViewModeWarning title:nil detail:@"您当前尝试在新设备上登录,确定要继续?" cencelBtn:@"取消" otherBtn:@[@"确定"] Onview:[UIApplication sharedApplication].keyWindow compleBlock:^(NSInteger index) {
                            if (index == 1) {
//                                [self dismissViewControllerAnimated:YES completion:nil];
                                UpdateDevIDViewController *updateView = [UpdateDevIDViewController new];
                                updateView.delegate=self;
                                updateView.phoneStr = [[NSUserDefaults standardUserDefaults] objectForKey:kUserID];
                                updateView.state = Present_Dis;
                                updateView.pushWayFlag=@"1";
                                BaseNavigationViewController *nav=[[BaseNavigationViewController alloc]initWithRootViewController:updateView];
//                                [[UIApplication sharedApplication].keyWindow.rootViewController presentViewController:nav animated:YES completion:nil];
                        ...
                        [[MBPAlertView sharedMBPTextView] showTextOnly:[UIApplication sharedApplication].keyWindow message:@"您当前的版本太低,为了您的使用体验请升级版本后再来体验^_^"];
                    } else {
                        [[MBPAlertView sharedMBPTextView] showTextOnly:[UIApplication sharedApplication].keyWindow message:[NSString stringWithFormat:@"%@",_loginParse.msg]];
                        [self dismissViewControllerAnimated:YES completion:^{
                            LoginViewController *loginView = [LoginViewController new];
                            BaseNavigationViewController *nav = [[BaseNavigationViewController alloc]initWithRootViewController:loginView];
                            [[UIApplication sharedApplication].keyWindow.rootViewController presentViewController:nav animated:YES completion:nil];
...
- (NSDictionary *)getParamDic
{
    ...
    DLog(@"%@",[[NSUserDefaults standardUserDefaults] objectForKey:kUserID]);
    DLog(@"%@",[[NSUserDefaults standardUserDefaults] objectForKey:kUserPass]);
    if ([Utility sharedUtility].userInfo.clientId && ![[Utility sharedUtility].userInfo.clientId isEqualToString:@""]) {
        return @{@"userName":[[NSUserDefaults standardUserDefaults] objectForKey:kUserID],
                 @"password":[[NSUserDefaults standardUserDefaults] objectForKey:kUserPass],
                 @"equipmentNum":[OpenUDID value],
                 @"version":app_Version,
                 @"platformSource":PLATFORM,
                 @"lastLoginIp":[[GetUserIP sharedUserIP] getIPAddress],
                 @"uuid":[Utility sharedUtility].userInfo.uuidStr,
                 @"clientId":[Utility sharedUtility].userInfo.clientId
                 };
    } else {
        return @{@"userName":[[NSUserDefaults standardUserDefaults] objectForKey:kUserID],
                 @"password":[[NSUserDefaults standardUserDefaults] objectForKey:kUserPass],
