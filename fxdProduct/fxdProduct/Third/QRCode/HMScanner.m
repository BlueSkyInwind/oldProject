...
+ (void)qrImageWithString:(NSString *)string avatar:(UIImage *)avatar scale:(CGFloat)scale completion:(void (^)(UIImage *))completion {
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{ completion([self createNonInterpolatedUIImageFormCIImage:transformedImage size:500 addAvatar:avatar scale:scale]); });
    });
}
...
#pragma mark 生成高清二维码
+ (UIImage *)createNonInterpolatedUIImageFormCIImage:(CIImage *)ciImage size:(CGFloat)widthAndHeight addAvatar:(UIImage *)avatar scale:(CGFloat)userScale
{
    ...
    //添加头像
    CGFloat screenScale = [UIScreen mainScreen].scale;
    CGRect rect = CGRectMake(0, 0, [UIImage imageWithCGImage:scaledImage].size.width * screenScale, [UIImage imageWithCGImage:scaledImage].size.height * screenScale);
    
    UIGraphicsBeginImageContextWithOptions(rect.size, YES, screenScale);
    
    [[UIImage imageWithCGImage:scaledImage] drawInRect:rect];
    
    CGSize avatarSize = CGSizeMake(rect.size.width * userScale, rect.size.height * userScale);
    CGFloat x = (rect.size.width - avatarSize.width) * 0.5;
    CGFloat y = (rect.size.height - avatarSize.height) * 0.5;
    [avatar drawInRect:CGRectMake(x, y, avatarSize.width, avatarSize.height)];
