        ...
        for (NSArray *array in arr) {
            if (array.count != 1)
            {
                if ([[array objectAtIndex:0] isEqualToString:@"无"]) {
                    tels = [array objectAtIndex:1];
                }
                else
                {
                    names = [array objectAtIndex:0];
                    tels = [array objectAtIndex:1];
                }
            }
            else
            {
                names = [array lastObject];
            }
            
            NSDictionary *dic = @{@"detail_name_":names,
                                  @"telphone_no_":tels};
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *telstring = [tel stringByReplacingOccurrencesOfString:@"-" withString:@""];
    DLog(@"%@%@", name,tel);
    if ([self.delegate respondsToSelector:@selector(GetContactName:TelPhone:andFlagTure:)]) {
        [self.delegate GetContactName:name TelPhone:telstring andFlagTure:_tagFlage];
    }
    [self.navigationController popViewControllerAnimated:YES];
}
